...
Webflow.define('lightbox', module.exports = function ($) {
  ...
  function supportOldLightboxJson(data) {
    ...
    // custom video adaptation
    // https%3A%2F%2Fwww.youtube.com%2Fembed%2FyNVLjsAvp3E%3Fautoplay%3D1%26feature%3Doembed&url=https%3A%2F%2Fwww.youtube.com%2Fwatch%3Fv%3DyNVLjsAvp3E&image=https%3A%2F%2Fi.ytimg.com%2Fvi%2FyNVLjsAvp3E%2Fhqdefault.jpg&key=c4e54deccf4d4ec997a64902e9a30300&autoplay=1&type=text%2Fhtml&schema=youtube
    if(data.items){
      if(data.items.length > 0){
        if(data.items[0].section == 'cover'){
          var video_url_id = $('#video_url').text().split('v=')[1];
          data.items[0].html = data.items[0].html.replace('video_url_id',video_url_id).replace('video_url_id',video_url_id).replace('video_url_id',video_url_id);
          
          data.items[0].originalUrl = $('#video_url').text();
          data.items[0].url = $('#video_url').text();
